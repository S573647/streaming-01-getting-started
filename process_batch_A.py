...
OUTPUT_FILE_K_NAME = "batchfile_2_kelvin.csv"
...
def convert_f_to_k(temp_f):
    ...
    All CSV values are read as strings.
    """
    logging.debug(f"Calling convert_f_to_k() with {temp_f}.")
    kelvin = round((float(temp_f) - 32.0) * 5.0 / 9.0 + 273.15, 2)
    logging.debug(f"Converted {temp_f}F to {kelvin}K.")
    return kelvin


def process_rows_in_k(input_file_name, output_file_name):
    ...
    with open(input_file_name, "r") as input_file:
        ...
        with open(output_file_name, "w", newline="") as output_file:
            ...
            for row in reader:
                ...
                TempK = convert_f_to_k(TempF)

                # Write the transformed data to the output file
                writer.writerow([Year, Month, Day, Time, TempK])


def process_rows_in_c(input_file_name, output_file_name):
    """Read from input file, convert temperature, and write to output file."""
    logging.info(f"Calling process_rows(): {input_file_name} to {output_file_name}.")
...
if __name__ == "__main__":
    try:
        ...
        logging.info("Converting F --> C")
        process_rows_in_c(INPUT_FILE_NAME, OUTPUT_FILE_NAME)
        logging.info("Converting F --> K")
        process_rows_in_k(INPUT_FILE_NAME, OUTPUT_FILE_K_NAME)
